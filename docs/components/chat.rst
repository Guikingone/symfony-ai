...
these conversations can be labelled as "forked" from the main chat, to fork the current messages stream,
the :class:`Symfony\\AI\\Chat\\Chat` expose a ``fork`` method::
    ...
    $forkedChat = $chat->fork(new MessageBag(), 'forked');
    $forkedChat->submit(Message::ofUser('Hello there'));

...
final class AgentWorkflowTest extends TestCase
{
    ...
    public function testAndSplitBranchFailureThrowsAndInterruptedForkIsRecorded()
    {
        ...
        $agentWorkflow = new AgentWorkflow(
            ...
            $executors,
            $store,
            parallelStrategy: new \Symfony\AI\Agent\Workflow\ParallelExecution\SequentialExecutionStrategy(),
        );

        // (a) run() must throw the branch failure.
        ...
        $this->assertNotNull($thrownException, 'run() must throw when a branch fails.');
